...
class HpServiceController extends Controller
{
       ...
       public function store(Request $request)
    {
        if ($request->method() == 'POST') {
            ...
            $hpservices->status = $request->input('status');
            $hpservices->show_on_hp = $request->input('show_on_hp');
            ...
            $hpservices->sort1 = $request->input('sort_1');
            $hpservices->sort2 = $request->input('sort_2');
            $hpservices->sort3 = $request->input('sort_3');
                
            $slug1 = Str::slug($request->input('heading_1'));
            $slug2 = Str::slug($request->input('heading_2'));
            $slug3 = Str::slug($request->input('heading_3'));
            $hpservices->slug1 =  $slug1;
            $hpservices->slug2 = $slug2;
            $hpservices->slug3 = $slug3;

            //Disable all hpservices if this new slider will show on home page    
            if($request->input('show_on_hp') == 1){
                //DB::table('hpservices')->update(['show_on_hp'=>0]);
            }  
    ...
    {
        if ($request->method() == 'POST') {
            $serviceExist = Hpservices::where('id', $id)->get();
            if($serviceExist){
            ...
            $hpservices->sort2 = $request->input('sort_2');
            $hpservices->sort3 = $request->input('sort_3');
            $hpservices->sort4 = $request->input('sort_4');
            $hpservices->sort5 = $request->input('sort_5');
            $hpservices->sort6 = $request->input('sort_6');
                
            $slug1 = Str::slug($request->input('heading_1'));
            $slug2 = Str::slug($request->input('heading_2'));
            $slug3 = Str::slug($request->input('heading_3'));
            $slug4 = Str::slug($request->input('heading_4'));
            $slug5 = Str::slug($request->input('heading_5'));
            $slug6 = Str::slug($request->input('heading_6'));
            
            $hpservices->slug1 =  $slug1;
            $hpservices->slug2 = $slug2;
            $hpservices->slug3 = $slug3;

            $hpservices->slug4 =  $slug4;
            $hpservices->slug5 = $slug5;
            $hpservices->slug6= $slug6;
         ...
         public function delete(Request $request)
    {
        if ($request->method() == 'POST') {
          ...
          public function hpserviceview($id)
    {
        $hpservices = Hpservices::where('id', $id)->first();
        return view('hpservice.viewhpservice', ['hpservices' => $hpservices]);
    }
