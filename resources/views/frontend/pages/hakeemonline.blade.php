...
<p>Now you no longer have to wait in long queues to see the hakeem. Instead, you can simply consult our certified hakeems to get medical advice from the comfort of your homes through the FameHospital Hospital Online hakeem service. This service helps patients who are unable to physically visit the hospital to consult with hakeems via an online platform. To book an appointment, please fill out the forms below and deposit your amount in the following bank account:
</p>
...
<h4><small><strong>Note:&nbsp;</strong>Please check the hakeem’s fees and clinic days using the fields below, before transferring the funds.</small></h4>*/ ?>
<h3 class="account-details"><span class="bankname">Hakeem's Appointment</span></h3>
...
<form action="{{route('checkAppointmentHakeem')}}" name="orderhakeemform" id="orderhakeemform" method="post" enctype="multipart/form-data">
            <input type="hidden" name="_token" id="csrf-token" value="{{ Session::token() }}" />
            <div class="row">
            <div class="col-sm-6">
                <div class="form-group">
                  <label>Hakeem/Consultant:</label><span id='hakeem_idError'></span>
                  <select name="hakeem_id" id="hakeem_id" class="form-control chk-empty">
                    <option value=''>Select Hakeem</option>
                  @if($hakeems)  
                    @foreach($hakeems as $hakeem)
                    <option value="{{$hakeem->id}}">{{$hakeem->hakeem_fullname}}</option>
                    @endforeach
                  @endif  
                  <select>
                </div>
