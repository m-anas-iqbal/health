...
<p>Now you no longer have to wait in long queues to see the Laboratory. Instead, you can simply consult our certified Laboratorys to get medical advice from the comfort of your homes through the FameHospital Hospital Online Laboratory service. This service helps patients who are unable to physically visit the hospital to consult with Laboratorys via an online platform. To book an appointment, please fill out the forms below and deposit your amount in the following bank account:
</p>

<h3 class="account-details">
  <span class="bankname">@if($laboratory) {{$laboratory->labority_fullname}} @endif</span>
</h3>



<h4><small><strong>Note:&nbsp;</strong>Please check the Laboratory’s fees and clinic days using the fields below, before transferring the funds.</small></h4>
<hr>
...
<form action="{{route('checkAppointmentLabority')}}" name="orderlaborityform" id="orderlaborityform" method="post" enctype="multipart/form-data">
            <input type="hidden" name="_token" id="csrf-token" value="{{ Session::token() }}" />
            <input type="hidden" name="laboratory_slug"  value="{{ Request::segment(2)}}" />

            <div class="row">
            <div class="col-sm-6">
                <div class="form-group">
                  <label>Labority/Consultant:</label><span id='labority_idError'></span>
                  <select name="labority_id" id="labority_id" class="form-control chk-empty">
                    <option value=''>Select Labority</option>
                  @if($laboritys)  
                    @foreach($laboritys as $labority)
                    <option value="{{$labority->id}}">{{$labority->labority_fullname}}</option>
                    @endforeach
                  @endif  
                  <select>
                </div>
              ...
              <div class="col-sm-6">
                <div class="form-group">
                <label>Labority's Timmings:</label>
...
<script>
  ...
  $(document).on('change', '#labority_id', function(){
    var labority_id = $(this).val();
    console.log(labority_id);
    $.ajax({
      type:'post',
      url:'getSpecialityOfLabority',
      data:{labority_id:labority_id ,_token: "{{ csrf_token() }}"},
      dataType:'JSON',
      success:function(data){
        ...
        if(data.labority_time_lists){
          var labority_time_lists = data.labority_time_lists;
          var dt_opt_list = '';
          //console.log(labority_time_lists);
          $.each(labority_time_lists, function(kk, vv){
              var day = '';
              switch(vv.day){
...
<script>
  ...
  console.log(password_req);
  //orderlaborityform validation
  $("#orderlaborityform").validate({
      rules: {
        patient_phone: {
              required: true,
          },
      ...
      errorPlacement: function (error, element) {
          if (element.attr("name") == "labority_id") {
              error.appendTo("#labority_idError");
          }
      ...
      messages: {
          ...
          labority_id: '[<span class="text-danger ">labority is Required</span>]',
